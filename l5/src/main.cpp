...
const int width = 4 * FACTOR;
const int height = 3 * FACTOR;
...
const glm::vec3 spherePosition1 = glm::vec3(0.25, 1, -5);

const glm::vec3 sphereColor2 = glm::vec3(0.0, 0.5, 1);
const float sphereRadius2 = 0.75;
const glm::vec3 spherePosition2 = glm::vec3(-0.25, 3, -3.5);

const float floorY = 0.0;

float cameraFov = glm::radians(60.0f);
const glm::vec3 cameraPos(0, 1, 2);

glm::vec3 lightPosition = glm::vec3(-2, 5, -2);
...
float calculateShadow(const glm::vec3 &point) {
    ...
    for (int i = 0; i < SAMPLES_PER_PIXEL; i++) {
        ...
        float _;
        if ( // 1e-3f is to prevent from self-shadowing
...
glm::vec3 traceRay(const glm::vec3 &rayOrigin, const glm::vec3 &rayDirection) {
    ...
    if (intersectFloor(rayOrigin, rayDirection, lightDistanceFloor)) {
        ...
        outputColor = FLOOR_COLOR * lightColor * intensity * antiShadowFactor;
    }

    if (intersectSphere(rayOrigin, rayDirection, lightDistanceSphere1, spherePosition1, sphereRadius1)) {
    ...
    if (intersectSphere(rayOrigin, rayDirection, lightDistanceSphere2, spherePosition2, sphereRadius2)) {
